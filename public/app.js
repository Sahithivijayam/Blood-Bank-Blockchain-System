...
const contractAddress = '0x3A6ccCB69Dc836e737B6A83cb0D7428f4026122f';
const contractABI = [
    {
      "inputs": [],
...
async function donateBlood() {
    try {
        ...
        const bloodType = mapBloodType(prompt('Enter the blood type (A, B, AB, O):')); // Prompt for blood type
		console.log(bloodType);
        const donationAmount = prompt('Enter the Units of blood to donate:'); // Prompt for amount
		...
		setDonationDetails(donorAddress, bloodType, amount);
	...
	function setDonationDetails(donorAddress, bloodType, amount) {
	...
	function displayStoredDonationDetails() {
		...
		const currAddress = getCurrentUserAddress();
		const mapTyoe = {
			0: 'A',
			1: 'B',
			2: 'AB',
		...
		if (donationDetailsList) {
			...
			for (let i = 0; i < localStorage.length; i++) {
				...
				if (key.startsWith('donation_')) {
					...
					if(donationData.donorAddress == currAddress){
						const listItem = document.createElement('li');
						listItem.textContent = `Donation from donor at ${donationData.donorAddress}. Blood Type: ${donationData.bloodType}, Units: ${donationData.amount}`;
						donationDetailsList.appendChild(listItem);
					}
					// Update total amount
	...
	async function requestBlood() {
		try {
			...
			const bloodType = mapBloodType(prompt('Enter blood type (A, B, AB, O):'));
			console.log(bloodType);
...
async function displayBloodRequests(targetElementId) {
	...
	async function fetchBloodData() {
		try {
			...
			console.log("Hi");
			console.log("Blood Bank Contract:", await bloodBankContract);
			console.log("Hello");
			const totalBloodDonated = await bloodBankContract.methods.getTotalBloodDonated().call();

			// Call the smart contract function to get the blood inventory for a specific blood type
			const bloodInventoryA = await bloodBankContract.methods.getBloodInventory(0).call(); // Assuming 0 represents BloodType A
			const bloodInventoryB = await bloodBankContract.methods.getBloodInventory(1).call(); // Assuming 0 represents BloodType B
			const bloodInventoryAB = await bloodBankContract.methods.getBloodInventory(2).call(); // Assuming 0 represents BloodType AB
			const bloodInventoryO = await bloodBankContract.methods.getBloodInventory(3).call(); // Assuming 0 represents BloodType O

           // Update the displayed values
			document.getElementById('totalBloodDonated').textContent = totalBloodDonated;
			document.getElementById('bloodInventoryA').textContent = bloodInventoryA;
			document.getElementById('bloodInventoryB').textContent = bloodInventoryB;
			document.getElementById('bloodInventoryAB').textContent = bloodInventoryAB;
			document.getElementById('bloodInventoryO').textContent = bloodInventoryO;
		} catch (error) {
